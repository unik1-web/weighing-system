...
type ViewAction =
  ...
  | { type: 'load_start' }
  | { type: 'load_success'; items: VescomWeighingItem[] }
  ...
  switch (action.type) {
    ...
    case 'load_success':
      return {
        ...
        loading: false,
        items: action.items,
        selectedKeys: action.items.map(vescomListKey),
      };
    case 'load_error':
      return {
        ...state,
...
function vescomListKey(item: VescomWeighingItem): string {
...
function vescomJournalKey(item: VescomWeighingItem): string {
  return ticketImportKey({
    gross_datetime: parseVescomDateTime(item.datetimebrutto),
    tare_datetime: parseVescomDateTime(item.datetimetara),
...
function normalizeItems(value: unknown): VescomWeighingItem[] {
...
export function VescomImportView({ onImported }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!settings.vescom_db_path.trim()) {
      dispatch({ type: 'load_error', error: 'Укажите путь к базе Vescom в настройках и сохраните их' });
      return;
    }
    ...
    try {
      ...
      logger.info('vescom', `Загружено записей: ${loadedItems.length}`, { date: state.date });
    } catch (err: unknown) {
      if (!mountedRef.current || requestId !== requestRef.current) return;
      dispatch({
        type: 'load_error',
        error: err instanceof Error ? err.message : 'Не удалось загрузить данные из Vescom',
      });
    }
  }, [state.date, settings.vescom_db_path, settings.vescom_db_password, settings.vescom_db_user]);

  const toggleAll = (checked: boolean) => {
    dispatch({
      type: 'toggle_all',
      keys: checked ? importableItems.map(vescomListKey) : [],
    });
  };
  ...
  const handleImport = () => {
    const selectedItems = state.items.filter(
      (item) => selectedKeySet.has(vescomListKey(item)) && !existingKeys.has(vescomJournalKey(item)),
    );
    ...
    pauseDatabaseSync();
    try {
      TicketStorage.createMany(selectedItems.map(buildTicketFromVescom));
    ...
    <div className="space-y-4">
      ...
      {!settings.vescom_db_path.trim() && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Укажите путь к базе Vescom в разделе «Настройки» и сохраните их.
        </div>
      )}
        ...
        <div className="max-h-[28rem] overflow-y-auto">
          {state.items.length === 0 ? (
            ...
            state.items.map((item) => {
              const key = vescomListKey(item);
              const alreadyImported = existingKeys.has(vescomJournalKey(item));
              return (
                <div key={key} className="border-b border-slate-100 px-4 py-3 text-sm last:border-b-0">
                  <div className="flex items-start gap-3">
                    <input
                      type="checkbox"
                      className="mt-1"
                      checked={selectedKeySet.has(key)}
                      disabled={alreadyImported || state.importing}
                      onChange={(e) => toggleItem(key, e.target.checked)}
                    />
                    <div className="min-w-0 flex-1 space-y-1">
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                        {item.vescom_id != null ? (
                          <span className="font-semibold text-slate-800">ID {item.vescom_id}</span>
                        ) : null}
