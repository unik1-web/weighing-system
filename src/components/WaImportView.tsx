...
function viewReducer(state: ViewState, action: ViewAction): ViewState {
  switch (action.type) {
    ...
    case 'load_success': {
      const keys = action.items.map(waImportKey);
      return {
        ...state,
        loading: false,
        items: action.items,
...
> {
  ...
  const noteParts = [`Импортировано из WA (uid: ${item.wa_uid})`];
...
function waConnectionParams(settings: ReturnType<typeof SettingsStorage.getAppSettings>): Record<string, string> {
  return {
    host: settings.wa_host.trim() || '127.0.0.1',
    port: String(settings.wa_port || 3306),
    database: settings.wa_database.trim() || 'wa',
    user: settings.wa_user.trim() || 'root',
...
export function WaImportView({ onImported }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!settings.wa_user.trim()) {
      dispatch({ type: 'load_error', error: 'Укажите пользователя MySQL в настройках и сохраните их' });
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            Загрузка взвешиваний из MySQL-базы программы WA (таблица autob).
          </p>
        </div>
      </div>
      ...
      {!settings.wa_user.trim() && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Укажите параметры подключения к MySQL в разделе «Настройки» и запустите backend: <code>npm run dev:api</code>
        </div>
      )}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1 space-y-1">
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                          <span className="font-semibold text-slate-800">uid {item.wa_uid}</span>
                        ...
                        <div className="text-slate-500">Водитель: {item.driver_name}</div>
