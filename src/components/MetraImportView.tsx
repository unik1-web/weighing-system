...
> {
  ...
  return {
    ...
    total_amount: item.price && item.net_weight ? (item.price * item.net_weight) / 1000 : null,
...
function normalizeItems(value: unknown): MetraWeighingItem[] {
...
export function MetraImportView({ onImported }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!settings.metra_db_path.trim()) {
      dispatch({ type: 'load_error', error: 'Укажите каталог базы Metra в настройках и сохраните их' });
      return;
    }
    ...
    try {
      ...
      dispatch({
        type: 'load_error',
        error: err instanceof Error ? err.message : 'Не удалось загрузить данные из Metra',
      });
    }
  }, [state.date, settings.metra_db_path]);

  const toggleAll = (checked: boolean) => {
    dispatch({
      type: 'toggle_all',
      keys: checked ? importableItems.map(metraImportKey) : [],
    });
  };
  ...
  const handleImport = async () => {
    const selectedItems = state.items.filter(
      (item) => selectedKeySet.has(metraImportKey(item)) && !existingKeys.has(metraJournalKey(item)),
    );
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1 space-y-1">
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                          <span className="font-semibold text-slate-800">№{item.rec_no}</span>
