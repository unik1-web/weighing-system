...
- **Импорт справочников** — из баз Vescom и Metra с нормализацией госномеров
...
При импорте Vescom/Metra номера нормализуются (латиница→кириллица, без пробелов); если регион не указан — подставляется **56**. Дубликаты не создаются. Повторный импорт той же взвешивающей записи (ключ: брутто + тара + госномер) в UI блокируется.
