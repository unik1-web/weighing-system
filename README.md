...
- **Импорт WA** — MySQL-база программы WA (таблица `autob`)
...
| MySQL Server | для WA (служба `MySQL80`, база `wa`) |
| pymysql | для WA (`pip install pymysql`, входит в `requirements.txt`) |
...
### WA (MySQL)
1. Включить импорт
2. Указать хост (`127.0.0.1`), порт (`3306`), базу (`wa`), пользователя и пароль MySQL
3. **Проверка WA** — тест подключения и подсчёт записей в `autob`
4. **Импорт справочников** — водители, грузы, контрагенты, автомобили из `lst_*`
5. Сохранить → вкладка **Импорт WA**

Данные WA хранятся в MySQL (каталог: `C:\ProgramData\MySQL\MySQL Server 8.0\Data\wa`). Для второй весовой может быть база `wa0`.
...
| WA / pymysql | `pip install pymysql`, служба MySQL80, пароль пользователя |
