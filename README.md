...
Локальное распознавание госномера по **обзорной** камере. По умолчанию выключено; взвешивание без ANPR работает как обычно.

#### Что нужно на объекте

1. **Полная сборка** с `onnxruntime` (в базовой строка статуса покажет «Модель: недоступна»).
2. Файл модели рядом с программой: `models/anpr/plate.onnx` (**не** в git; каталог `models/` в `.gitignore`).
3. Настроенные камеры: **видеофикация** включена, есть камера с ролью **обзор** и рабочим URL; желательно ROI на зону номера.
4. Активный комплект весов — **основные** (primary). На **резерве** движок не вызывается (`anpr_mode=disabled_by_configuration`).
...
| Там же | Строка **«Модель: …»** | `загружена` — файл и runtime на месте; `недоступна` — нет полной сборки и/или `plate.onnx` |
...
После изменения флагов нажмите **Сохранить настройки**. Ключи пишутся в `config.ini` (`video_enabled`, `anpr_enabled`).

Включать **ANPR** в прод имеет смысл **после спайка** на объекте с точностью ≥ 50% (чеклист: `docs/implementation/anpr-spike-checklist.md`). Иначе оставьте выкл. и вводите номер вручную.

#### На форме взвешивания

1. Убедитесь, что индикатор/статус ANPR не говорит об отключении на резерве.
2. Нажмите **«Распознать номер»** — backend снимает overview (+ ROI) и запускает инференс; тикет сам по себе не пишется.
3. Примите результат, поправьте номер или введите вручную — заполняются `plate_source` / поля ANPR на провеске.

При сбое камеры, отсутствии модели или ошибке движка статус `failed` (или аналог): взвешивание продолжается вручную.

#### Журнал

Фильтр **ANPR** и колонка/карточка статуса (`anpr_status`, сырой номер, confidence при наличии). Источник номера и ANPR-поля уходят в CSV вместе с остальным аудитом.

Модель и кадры с номерами в репозиторий не коммитьте.
