...
"""PyInstaller spec for the full Windows delivery (cameras + OpenCV)."""
...
hiddenimports = [
    ...
    'vescom',
    'wa',
    # Stage-7 camera/photo modules (full build bundles OpenCV for RTSP).
    'cameras',
    'camera_logging',
    'photo_storage',
    'ticket_photos',
...
# Pull OpenCV binary/data hooks so RTSP capture works in the frozen exe.
try:
    cv2_datas, cv2_binaries, cv2_hidden = collect_all('cv2')
    datas += cv2_datas
    binaries += cv2_binaries
    hiddenimports += list(cv2_hidden)
except Exception:
    pass
