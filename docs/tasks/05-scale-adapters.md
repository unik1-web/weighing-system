...
- [ ] Существующие 4 профиля работают через адаптеры без регрессии чтения веса.
- [ ] Новый терминал подключается конфигом (regex/маска) или тонким адаптером.
- [ ] Активный комплект primary/spare выбирает своё подключение.
- [ ] Есть зачаток или рабочий путь `/api/scales/*` / backend I/O (минимум — контракт + один транспорт; объём уточнит ТЗ).
- [ ] Ручной ввод с фиксацией источника; причина — по правилам настроек.
- [ ] Регрессии: форма single/dual, `stable_mode`, журнал, печать.
- [ ] Тесты парсеров адаптеров и выбора активного комплекта.
